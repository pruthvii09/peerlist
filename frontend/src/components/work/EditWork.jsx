...
import useFetchSingleWork from "../../hooks/work/useGetSingleWork";
import { useParams } from "react-router-dom";
import { Loader2 } from "lucide-react";
import { useEditWork } from "../../hooks/work/useEditPost";
const EditWork = () => {
  const modules = {
    toolbar: [
  ...
  const { data, isLoading } = useFetchSingleWork(expId);

  // Split both start_date and end_date when initializing state
  const [workData, setWorkData] = useState(() => {
    if (data) {
      const [startMonth, startYear] = (data.start_date || "").split(" ");
      const [endMonth, endYear] = (data.end_date || "").split(" ");
      return {
        ...data,
        startMonth,
        startYear,
  ...
  const handleStartMonthChange = (e) => {
    ...
    setWorkData((prev) => ({
      ...prev,
      startMonth: newMonth,
      start_date: `${newMonth} ${prev.startYear || ""}`.trim(),
    }));
  };
  ...
  const handleStartYearChange = (e) => {
    ...
    setWorkData((prev) => ({
      ...prev,
      startYear: newYear,
      start_date: `${prev.startMonth || ""} ${newYear}`.trim(),
    }));
  };
  ...
  const handleEndMonthChange = (e) => {
    ...
    setWorkData((prev) => ({
      ...prev,
      endMonth: newMonth,
      end_date: `${newMonth} ${prev.endYear || ""}`.trim(),
    }));
  };
  ...
  const handleEndYearChange = (e) => {
    ...
    setWorkData((prev) => ({
      ...prev,
      endYear: newYear,
      end_date: `${prev.endMonth || ""} ${newYear}`.trim(),
    }));
  };
    ...
    <div className="mt-14 border-r h-full pb-14">
      ...
      <div className="fixed max-w-[640px] w-full sm:bottom-0 bottom-16 flex items-center justify-end px-6 py-4 bg-[#f6f8fa] border-t border-r">
        <Button
          title="Save"
          onClick={handleSave}
          loading={editWorkMutation.isPending}
